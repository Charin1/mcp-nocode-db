...
async def get_saved_queries(current_user: User = Depends(get_current_user)):
    try:
        return AuditService.get_saved_queries(username=current_user.username)
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Failed to fetch saved queries: {e}")

@router.delete("/saved-queries/{query_id}")
async def delete_saved_query(query_id: int, current_user: User = Depends(get_current_user)):
    try:
        AuditService.delete_saved_query(query_id=query_id, username=current_user.username)
